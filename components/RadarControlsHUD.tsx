...
import { Radio, Crosshair, Cpu, Volume2, VolumeX, RotateCcw, X, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-20 flex flex-col justify-between p-2 sm:p-4 select-none font-mono text-emerald-400">
      ...
      <div className="w-full flex items-center justify-between gap-2 bg-black/90 border border-emerald-500/50 px-3 py-2 rounded-xl backdrop-blur-md pointer-events-auto text-xs shadow-[0_0_20px_rgba(0,0,0,0.8)]">
        ...
        {/* Left: Back / Main Menu & Location */}
        <div className="flex items-center gap-2.5">
          <button
            onClick={onReturnToStart}
            className="px-2.5 py-1 rounded-lg bg-zinc-900 hover:bg-emerald-950 border border-emerald-700 hover:border-emerald-400 text-emerald-300 hover:text-white flex items-center gap-1.5 transition-all cursor-pointer text-[11px] font-bold shadow-[0_0_10px_rgba(16,185,129,0.2)]"
            title="Return to Main Menu / Select New City (Esc)"
          >
        ...
        <div className="hidden sm:flex items-center gap-2 sm:gap-3 text-[10px] sm:text-[11px]">
        ...
        <div className="flex items-center gap-2">
          <div className="sm:hidden flex items-center gap-1.5 text-[10px]">
            <span>INT: <strong className="text-emerald-300">{stats.intercepted}</strong></span>
            <span>HIT: <strong className="text-red-400">{stats.impacts}</strong></span>
          </div>

          <button
            onClick={onToggleMute}
            className="p-1.5 rounded-lg bg-zinc-900 border border-zinc-700 hover:border-emerald-500 text-zinc-300 hover:text-emerald-300 transition-all cursor-pointer"
