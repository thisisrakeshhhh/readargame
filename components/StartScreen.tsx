...
import { Search, MapPin, Navigation, Radio, ArrowRight } from 'lucide-react';
...
export const StartScreen: React.FC<StartScreenProps> = ({ onEnterRadar }) => {
  ...
  const filteredLocations = searchQuery.trim() ? searchLocations(searchQuery) : WORLD_LOCATIONS.slice(0, 8);
  ...
  return (
    <div className="w-full h-full min-h-screen bg-black text-emerald-400 font-mono flex flex-col justify-center items-center p-4 select-none relative overflow-hidden">
      ...
      {/* Main Container */}
      <div className="max-w-md w-full bg-zinc-950/90 border border-emerald-500/40 p-6 md:p-8 rounded-2xl shadow-[0_0_40px_rgba(16,185,129,0.15)] z-10 space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search country, city or location (e.g. India, Kyiv, Russia)..."
          ...
          <div className="flex gap-1.5 overflow-x-auto py-1">
            {['India', 'Ukraine', 'Russia', 'Iran', 'USA', 'Japan', 'UK'].map((c) => (
              <button
                key={c}
                onClick={() => setSearchQuery(c)}
                className="px-2 py-0.5 rounded bg-emerald-950/60 border border-emerald-900 hover:border-emerald-500 text-[10px] text-emerald-400 hover:text-emerald-200 transition-all cursor-pointer shrink-0"
              >
                {c}
              </button>
            ))}
        ...
        <div className="space-y-1.5 max-h-44 overflow-y-auto pr-1 border border-emerald-950/80 rounded-xl p-1 bg-black/60 custom-scrollbar">
          {filteredLocations.length > 0 ? (
              ...
              return (
                <div
                  ...
                  className={`p-2 rounded-lg cursor-pointer transition-all flex items-center justify-between text-xs ${
                    isSelected
                      ? 'bg-emerald-950/80 border border-emerald-400 text-emerald-200 shadow-[0_0_10px_rgba(52,211,153,0.3)]'
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-emerald-600">{loc.country}</div>
                    </div>
                  </div>
            ...
            <div className="text-center py-4 text-xs text-emerald-700">No matching locations found</div>
          )}
        </div>
